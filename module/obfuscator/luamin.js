    ...
    function localdecl(locals, upvals) {
        ...
        if (peek().Source == "function") {
            ...
            if (funcStat.NameChain.length > 1) {
                throw (getTokenStartPosition(funcStat.Token_NameChainSeperator[0]) +
                    ": `(` expected.");
            }
            let node;
            node = MkNode({
...
function FormatAst(ast) {
    ...
    formatExpr = function (expr) {
        if (expr.Type == "BinopExpr") {
            ...
            if (expr.Type == "HashLiteral") {
                expr.Token.Source =
                    '"' +
                        hashString(`${expr.Token.Source.substring(1, expr.Token.Source.length - 1)}`) +
                        '"';
                expr.Type = "StringLiteral";
    ...
    formatStat = function (stat) {
        if (stat.Type == "StatList") {
            ...
            if (stat.FunctionStat.ArgList.length > 0 &&
...
function StripAst(ast) {
    ...
    function joint(tokenA, tokenB, shit = false) {
        stript(tokenB);
        let lastCh = (typeof tokenA.Source == "string"
            ? tokenA.Source
            : tokenA.Source.toString()).substr(tokenA.Source.length - 1, 1);
        let firstCh = (typeof tokenB.Source == "string"
            ? tokenB.Source
            : tokenB.Source.toString()).substr(0, 1);
        if ((lastCh == "-" && firstCh == "-") ||
...
function SolveMath(ast) {
    ...
    function removething(a) {
        if (a == null || a.substr == null)
            return;
        let start = a.substr(0, 1);
        let ret;
        if (start == `"` || start == `'`)
            ret = a.substr(1, a.length - 2);
        if (start == `[`) {
            let count = 0;
            let p = 1;
            while (a.substr(p, 1) == "=") {
    ...
    function solvebinop(operator, left1, right1) {
        let lhs = left1;
        let rhs = right1;
        if (left1 && left1.Type == "ParenExpr")
            lhs = left1.Expression;
        if (right1 && right1.Type == "ParenExpr")
        ...
        let left;
        let right;
        if (lhs.Type == "BooleanLiteral")
            left = lSrc == "true" ? true : false;
        if (rhs.Type == "BooleanLiteral")
            right = rSrc == "true" ? true : false;
        ...
        if (left != null && right != null) {
            ...
            if (lhs.Type == "StringLiteral")
                left = parseFloat(removething(left));
            if (rhs.Type == "StringLiteral")
                right = parseFloat(removething(right));
            if (left == null || right == null)
                return;
            let val;
            if (operator == "+")
    ...
    function solveunop(operator, rhs) {
        ...
        if (b == null)
            return;
        if (b.Source == null && rhs.Type != "TableLiteral")
            return;
            ...
            b.forEach((v, i) => {
                if (ignoreRest) {
                    ...
                    if (v.EntryType == "Value" || v.EntryType == "Index") {
                        if ((v.Index == null || v.Index.Type == "NumberLiteral") &&
                            v.Value) {
                            let index = v.Index != null &&
                                v.Index.Token != null &&
        ...
        if (rhs.Type == "StringLiteral")
            right = rSrc.substr(1, rSrc.length - 2);
    ...
    solveStat = function (stat) {
        if (stat.Type == "StatList") {
            ...
            switch (condition.Type) {
                ...
                case "BooleanLiteral": {
                    if (stat.ElseClauseList.length >= 1 ||
                        condition == null ||
                        condition.Token == null ||
                        condition.Token.Source !== "false") {
                        break;
...
function Uglify(ast) {
    ...
    function turnNumberToFTable(value1) {
        ...
        let value = a[0];
        let dec = a[1];
        let leftover = value - Math.min(value, maxtablelength);
        ...
        let rancommas = [{ Type: "Symbol", Source: ",", LeadingWhite: " " }];
        for (let i = 0; i > 2; i++) {
            let ran = Math.random();
            if (ran > 0.5) {
                args.push({
                    Type: "Ident",
                    Source: `d${Math.floor(Math.random() * 50)}`,
                    LeadingWhite: " ",
                });
    ...
    function turnNumberToFString(value) {
        ...
        let leftover = value - Math.min(value, maxstrlength);
    ...
    uglifyExpr = function (expr, uglied) {
        if (expr.Type == "BinopExpr") {
            ...
            switch (expr.Type) {
                case "NumberLiteral": {
                    let value = isFinite(`0${expr.Token.Source}`) &&
                        parseFloat(`0${expr.Token.Source}`);
                ...
                case "StringLiteral": {
                    ...
                    if (howtofuckup == 0) {
                        ...
                        if (!addedLocal) {
                            addedLocal = true;
                            let local = CreateLuaParser(`local string = string`)
                                .StatementList[0];
    ...
    uglifyStat = function (stat, uglied) {
        if (stat.Type == "StatList") {
            ...
            for (i = 0; i <= stat.StatementList.length; i++) {
                ...
                if (lastChStat != null) {
                    if ((stat.SemicolonList[i - 1] &&
                        lastChStat.GetLastToken().Source != ")") ||
                        chStat.GetFirstToken().Source != ")") {
                        stat.SemicolonList[i - 1] = null;
                    }
                    if (!stat.SemicolonList[i - 1]) {
    ...
    constTokens.forEach((v, i) => {
        ...
        if (taken[v.Type][v.Token.Source]) {
            ...
            consttable += (A++ == 0 ? "" : ", ") + `${ranName} = ${v.Token.Source}`;
                ...
                if (!otherVar.Renamed) {
                    if (!otherVar.Scope || otherVar.Scope.Depth < _var.Scope.Depth) {
                        ...
                        _var.ReferenceLocationList.some((refAt) => {
                            if (refAt >= otherVar.BeginLocation &&
                        ...
                        if (_var.BeginLocation < otherVar.EndLocation &&
...
let watermark = `--Minified By github.com/BloxSafe\n\n`;
let luaminp = {};
...
luaminp.Uglify = function (src1, options) {
    ...
    glb1.forEach((v) => {
        ...
        ].concat(ast1.SemicolonList);
        lol = [
            CreateLuaParser(`local ${v.Name} = getfenv()['${v.Name}']`)
                .StatementList[0],
        ].concat(lol);
    });
    glb1 = null;
