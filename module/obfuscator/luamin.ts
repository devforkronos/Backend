...
function generateUglyName(l = 20) {
  ...
  let gamer = gen();
  while (uglyNames.includes(gamer)) gamer = gen();
...
function Uglify(ast) {
  ...
  let constTokens = [];
  function stript(token) {
    if (token != null) token.LeadingWhite = "";
  }
  ...
  function generateRandomShit(ignore, value = 10) {
    ...
    if (num == 0) {
      str = `${rannum()} and true or ${randomShit(
        "(a)"
      )} * (b)((false)(${randomShit()})("" + ".." .. ${randomShit()} .. "")(${randomShit()})) ^ ${randomShit()}`;
    } else if (num == 1 && !ignore) {
      str = `(function(a,b,c)
                return ${generateRandomShit(true)} 
                    and a or a == nil or b * a + b, c end)('${genHexString(
      ...
      switch (expr.Type) {
        ...
        case "StringLiteral": {
          ...
          if (howtofuckup == 0) {
            ...
            let callexpr = CreateLuaParser(
              `_ = string.char(${a.concat()}) .. ${start}${b}${end}`
            ).StatementList[0].Rhs[0];
            if (!addedLocal) {
              addedLocal = true;
